...
The pipeline is a Python 3.13+ package managed with the uv package manager (for deterministic lock-file resolution) and exposed through a single command-line utility that is also importable as a Python module. The code is organised as one package per stage (sourcing, ingestion, cleaning, exploratory analysis, normalisation, and dashboard), each with a corresponding test module. The source catalogue is a single version-controlled spreadsheet. All working directories, that is, temporary or intermediate artifact locations, resolve from a single environment variable that defaults to the current working directory, allowing a code-only repository whose deployment simply points that variable elsewhere. Secrets are read only from an environment file and never encoded into the source. A streaming entry point drives per-source ingestion: a run command executes only until cross-source deduplication completes, whereas an all command continues through normalisation. A resume option lets both commands continue from any interruption by reading the completion ledger, and all generated outputs are excluded from version control.
...
A single streaming entry point drives the build: per-source ingest and clean (isolated, so one failing source cannot halt the run), cross-source deduplication, the sufficiency gate, and normalisation. No external scheduler or dataset-versioning service is required, which keeps the pipeline reproducible from the repository alone. Traceability is carried by the provenance manifest rather than by an external version store: every record records its source and licence, and the manifest records a content hash of the released dataset, which is what permits a bad batch to be scoped and withdrawn. Deployment is Dockerised, and the image runs as an unprivileged user, writes only to a mounted output volume, and carries no embedded credentials, reading secrets from the environment at run time.
...
Release & Provenance manifest (datasheet) with dataset content hashing, enabling scoped rollback. \\ \hline
