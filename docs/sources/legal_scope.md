...
|---|---|---|
| `rbi.org.in` | "© Reserve Bank of India. **All Rights Reserved**". No reproduction grant on the copyright page or the disclaimer. The disclaimer requires prior written permission even to deep-link. `robots.txt` answers automated clients with **HTTP 418**. | **Barred for corpus text.** `all rights reserved` is a deny pattern in `license_gate._DENY`, so these rows would be blocked at ingestion even if discovered. RBI publishes an RSS feed (`/Scripts/rss.aspx`) which *is* meant for automated readers — tracked as a metadata-only update index (title/date/URL, no document text), which raises no reproduction question. |
