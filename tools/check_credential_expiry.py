...
Run this on a schedule (see .github/workflows/credential-expiry.yml) instead
of relying on someone remembering to rotate crawler credentials. It decrypts
the same file core.py reads at pipeline startup and only ever prints source
names and dates -- never a credential value.

Exit status: 0 if every credential is fine, 1 if anything needs rotating (so
this can gate a CI job / send an alert).
"""
...
def main() -> int:
    ...
    for source, fields in creds.items():
        ...
        if not expires_at:
            # This provider doesn't support expiring credentials (e.g. NVD,
            # Kaggle today) -- rotation for these is a calendar reminder, not
            # something this check can enforce. rotated_at is still recorded
            # in the file so a human can eyeball how stale it is.
