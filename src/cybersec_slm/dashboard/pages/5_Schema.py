...
st.caption("Cleaned records mapped onto the canonical schema in "
           "`data/final/dataset.jsonl`, with a provenance manifest, then browse "
           "the result below.")
st.divider()
...
man = data.manifest()
if not man:
    st.caption("No manifest yet (`data/final/manifest.json`). Run this stage from "
               "the Overview page.")
else:
    ui.stat_grid([
...
st.caption(f"showing {shown_lo} to {shown_hi} of {result['match_count']}{more} matches"
